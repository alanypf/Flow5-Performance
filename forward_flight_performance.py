"""Forward-flight performance analysis for tailsitters.

Sweeps cruise, climb, V_max (powertrain limit), control authority
(differential thrust on an X-quad) and characteristic-speeds-vs-altitude
for a fixed airframe + propulsion stack. Hover (V = 0) lives in
hover_performance.py.
...
Assumptions worth knowing:
  ...
  * Usable battery energy = V_nominal * capacity * usable_fraction.
"""
...
if TYPE_CHECKING:
    from hover_performance import HoverPoint
...
def airframe_drag(polar: dict, mass: float, area: float, rho: float,
    ...
    L = W so CL_req = 2W/(rho*V^2*S); CD is interpolated from the
    pre-stall branch. Returns NaN where the airframe cannot hold weight
    (V below stall or above a polar that doesn't reach low-enough CL).
    """
...
def alpha_level_flight(polar: dict, mass: float, area: float, rho: float,
    ...
    Inverts the pre-stall CL(alpha) branch to find alpha such that
    CL = 2W/(rho*V^2*S). Returns NaN where CL_req is outside the polar.
    """
    alpha = np.asarray(polar["alpha"], dtype=float)
    cl = np.asarray(polar["cl"], dtype=float)
    order = np.argsort(alpha)
...
def isa_density(altitude_m: float) -> float:
    """ISA air density [kg/m^3] for altitude below 11 000 m (troposphere)."""
...
# ---------------------------------------------------------------------------
# Throttle root-finder (shared with hover at V = 0)
# ---------------------------------------------------------------------------
...
def _find_throttle_for_thrust(motor: Motor, prop: Propeller, battery: Battery,
    ...
    """Bisect throttle in [0, 1] so per-motor prop thrust at V equals T_req.
...
class ControlAuthorityPoint:
    ...
    dT_max_N: float            # max dT per motor (symmetric about trim)
    ...
    alpha_roll_rads2: float    # max roll angular acceleration [rad/s^2]
    alpha_pitch_rads2: float   # max pitch angular acceleration [rad/s^2]
...
def control_authority_sweep(
    ...
    For each V the available dT per motor is limited by whichever is smaller:
    ...
    for V, D in zip(V_array, D_arr):
        ...
        # Symmetric dT budget: can't exceed T_max or go below 0
...
def print_forward_section(plane: dict, polar: dict, motor: Motor, prop: Propeller,
                          battery: Battery, n_motors: int,
                          cruise: list[CruisePoint | None],
                          climb: list[ClimbPoint | None],
                          ctrl: list[ControlAuthorityPoint | None] | None = None,
                          hover: "HoverPoint | None" = None) -> None:
    """Print the CRUISE / CLIMB / T-vs-W / CONTROL AUTHORITY blocks.

    ``hover`` is optional and only used to prepend the V=0 row to the
    T/W-vs-airspeed table.
    """
    mass = plane["mass"]
    g = plane.get("gravity", 9.81)
    W = mass * g
        ...
        if valid_ctrl:
            ...
            if arm is not None:
                print(f"  Motor arm       : {arm:.3f} m  "
                      f"(dihedral = {dihedral_deg:.0f} deg, "
...
def print_altitude_speeds(polar: dict, mass: float, area: float,
                          g: float = 9.81) -> None:
    """Print stall, best-endurance, best-range speeds every 500 m up to 3000 m."""
    alt_pts = altitude_speed_sweep(polar, mass, area, g=g)
    print("\n--- CHARACTERISTIC SPEEDS vs ALTITUDE ---")
    print("   Alt [m]   rho [kg/m^3]  V_stall  V_endurance  V_best_range")
    print("  " + "-" * 58)
...
def plot_altitude_speeds(polar: dict, mass: float, area: float,
    ...
    ax1.set_title(f"Characteristic speeds vs altitude  (m={mass:.2f} kg, S={area:.4f} m^2)")
    ax1.grid(True, alpha=0.3)
    ax1.legend(loc="upper left", fontsize=9)

    ax2 = ax1.twinx()
    ax2.plot(alt, rho_arr, "--", color="grey", alpha=0.5, linewidth=1)
    ax2.set_ylabel("Air density [kg/m^3]", color="grey")
...
def plot_motor_rpm_vs_airspeed(motor: Motor, battery: Battery,
    ...
    ``hover`` overlay (horizontal line at hover RPM) is drawn when provided.
    """
    import matplotlib.pyplot as plt
...
def plot_forward_performance(plane: dict, polar: dict, motor: Motor, prop: Propeller,
                             battery: Battery, n_motors: int,
                             cruise: list[CruisePoint | None],
                             climb: list[ClimbPoint | None],
                             save_path: str,
                             ...
                             ctrl: list[ControlAuthorityPoint | None] | None = None,
                             hover: "HoverPoint | None" = None) -> None:
    """Big 2- or 3-row figure summarising forward-flight performance.

    ``hover`` is optional. When provided the V=0 hover point is overlaid
    as markers / horizontal references on the relevant panels.
    """
    import matplotlib.pyplot as plt
    ...
    fig.suptitle(
        ...
        f"|  m={mass:.2f} kg  S={area:.3f} m^2  N={n_motors}\n{hover_str}",
        fontsize=11,
    )
    ...
    if n_rows == 3:
        ...
        # 9) Available moment vs airspeed
        ax = axes[2, 0]
        ...
        ax.set_xlabel("Airspeed [m/s]")
        ax.set_ylabel("Max angular accel [deg/s^2]")
        ...
        ai.plot(V_ca, np.degrees(a_pitch), "s-", color="tab:red",
                label=f"Pitch (Iyy={plane.get('inertia',{}).get('Iyy','?')})")
        ai.set_xlabel("Airspeed [m/s]"); ai.set_ylabel("Max angular accel [deg/s^2]")
        ...
        ax.fill_between(V_ca, T_trim - dT, T_trim + dT,
                        alpha=0.25, color="tab:green", label="+/-dT envelope")
